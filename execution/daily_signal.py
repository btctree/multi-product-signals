#!/usr/bin/env python3
"""Daily action + P&L digest to Telegram, for MANUAL execution while the IB
gateway cannot log in (IBKR forced passkey 2FA, 2026-08-24).
...
Position quantities come from data/bot_state.json, which is frozen at the last
successful publish. Any fill after that timestamp is invisible here; the message
...
def yahoo(sym):
    ...
    (e.g. Tokyo names, whose cards are only generated for published holdings)."""
    try:
...
def fx_rates():
    """<ccy> -> HKD. Live where possible, documented fallbacks otherwise."""
    out = {"HKD": 1.0}
    usdhkd = None
    try:
...
def main():
    ...
    try:
        ...
        log("FATAL: cannot read %s: %s" % (ENVF, e))
        return 2
    token, chat = cfg.get("TELEGRAM_TOKEN"), cfg.get("TELEGRAM_CHAT_ID")
    if not token or not chat:
        log("FATAL: TELEGRAM_TOKEN / TELEGRAM_CHAT_ID missing")
        return 2
    ...
    try:
        ...
        for a in (d.get("actions") or []):
            ...
            if shares <= 0:
                ...
                continue
            buys.append({"ysym": ysym, "shares": shares, "price": price, "ccy": ccy,
    ...
    L = ["<b>\U0001F4CB ACTION LIST — %s</b>" % today,
         "<i>Manual mode — IB gateway down. Your strategy's own rules, replayed offline. You place them.</i>", ""]
    ...
    if buys:
        ...
        L.append("   none" + ("  <i>(kill switch active)</i>" if killed else
                              ("  <i>(no free slot)</i>" if free <= 0 else "")))
    L.append("")
    ...
    L.append("<b>\U0001F4B0 P&amp;L</b>")
    L.append("Est. NetLiq  <b>HK$%s</b>" % money(est_netliq))
    if day_pnl is not None:
        L.append("Today        <b>%s</b> HKD" % sign(day_pnl))
    else:
        L.append("Today        <i>(first run — baseline set)</i>")
    if overall is not None:
        L.append("Since launch <b>%s</b> HKD (%+.2f%%)  <i>flows excluded</i>" % (sign(overall), overall_pct))
    L.append("Unrealised   %s HKD on HK$%s cost" % (sign(mv_hkd - cost_hkd), money(cost_hkd)))
    L.append("")
    ...
    for h in sorted(holds + exits, key=lambda x: (x["head"] is None, x["head"])):
        ...
        flag = " ⚠️" if h["reason"] else ""
        L.append("<code>%-7s %8.2f  stop %8.2f  %6s  %+.1f%%</code>%s"
                 % (h["ysym"], h["price"], h["stop"], hd, h["upl_pct"], flag))
    L.append("<i>cols: price, stop, headroom, unrealised</i>")
    L.append("")
    ...
    try:
        with urllib.request.urlopen(req, timeout=30) as r:
            resp = json.load(r)
        if not resp.get("ok"):
            log("telegram rejected: %s" % json.dumps(resp)[:300])
            return 1
        log("sent message_id %s" % resp["result"]["message_id"])
    except Exception as e:
    ...
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump({"date": datetime.date.today().isoformat(), "est_netliq": est_netliq,
                   "mv_hkd": mv_hkd, "cost_hkd": cost_hkd}, f)
    os.replace(tmp, PREV)
    return 0
